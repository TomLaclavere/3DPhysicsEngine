...
TEST(QuaternionTest, FromMatrix_IntegerBranches)
{
    // Case 1 (i=1 because m11 > m00)
    // 180° around Y:  R = diag(-1, 1, -1), expected q = (0,1,0,0)
    {
        Matrix3x3    R(-1_d, 0_d, 0_d, 0_d, 1_d, 0_d, 0_d, 0_d, -1_d);
        Quaternion3D q(R);
        EXPECT_DECIMAL_EQ(q.getRealPart(), 0_d);
        EXPECT_VECTOR_EQ(q.getImaginaryPart(), Vector3D(0_d, 1_d, 0_d));
    }

    // Case 2 / default (i=2 because m22 > m00/m11)
    // 180° around Z:  R = diag(-1,-1, 1), expected q = (0,0,1,0)
    {
        Matrix3x3    R(-1_d, 0_d, 0_d, 0_d, -1_d, 0_d, 0_d, 0_d, 1_d);
        Quaternion3D q(R);
        EXPECT_DECIMAL_EQ(q.getRealPart(), 0_d);
        EXPECT_VECTOR_EQ(q.getImaginaryPart(), Vector3D(0_d, 0_d, 1_d));
    }
}

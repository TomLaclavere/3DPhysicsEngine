...
SimResult simulation(const std::string& solver, decimal timestep, int maxiter, bool recordEnergy = false)
{
    ...
    while (counter < maxIter && world.getIsRunning())
    {
        ...
        {
            const decimal expected = analyticalPeakHeight(z0, radius, restitution, result.bounceCount);
            const decimal err = (expected > 0_d) ? commonMaths::absVal((currentPeak - expected) / expected)
...
{
    const std::string outputPath = "benchmarks/physics/Bouncing_Conservative/results";
        ...
        for (size_t iS = 0; iS < solvers.size(); ++iS)
        {
            for (size_t jDt = 0; jDt < N_DT; ++jDt)
            {
                ...
                file << solvers[iS] << "," << timesteps[jDt] << "," << r.maxHeightError << ","
                     << r.maxEnergyDrift << "," << r.finalEnergyDrift << "," << r.cpuUs << "," << opsTotal
                     << "," << r.bounceCount << "," << r.maxFlightEnergyDrift << "\n";
            }
        }
    }
